...
(function ($) {
	$.fn.tagedit = function (options) {
		...
		if (options.autocompleteURL) options.autocompleteOptions = $.extend({
		...
		function inputsToList() {
			...
			.find('#tagedit-input').each(function () {
				...
				case 'SPAN':
				case 'LI':
					if ($(event.target).hasClass('tagedit-listelement-deleted') == false && $(event.target).parent('li').hasClass('tagedit-listelement-deleted') == false) {
						// Don't edit an deleted Items
						return doEdit(event);
					}
				default:
					$(this).find('#tagedit-input').removeAttr('disabled').removeClass('tagedit-input-disabled').focus();
				}
				return false;
			})
		}
		...
		function doEdit(event) {
			...
			if (options.allowDelete == true && element.find(':hidden').length > 0 && typeof element.find(':hidden').attr('name').match(baseNameRegexp)[3] != 'undefined') {
			...
			element.addClass('tagedit-listelement-edit').find('a.tagedit-save').click(function () {
				...
				window.clearTimeout(closeTimer);
				return false;
			}).end().find(':text').focus().autoGrowInput({
				comfortZone: 10,
				minWidth: 15,
				maxWidth: 20000
			}).keypress(function (event) {
				...
				closeTimer = window.setTimeout(function () {
					that.parent().trigger('finishEdit', [true])
				}, 500);
			});
		}
		/**
		...
		function markAsDeleted(element) {
			element.trigger('finishEdit', [true]).addClass('tagedit-listelement-deleted').attr('title', options.deletedElementTitle);
